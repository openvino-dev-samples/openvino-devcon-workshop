...
def get_llm_selection_widget(
    languages=list(SUPPORTED_LLM_MODELS), models=SUPPORTED_LLM_MODELS[default_language], show_preconverted_checkbox=False, device=None, default_model_idx=0
):
...
def convert_and_compress_model(model_id, model_config, precision, use_preconverted=False):
    ...
    ms_model_dir = snapshot_download(pt_model_id, cache_dir='.')
    optimum_cli_command = get_optimum_cli_command(ms_model_dir, weight_format, model_dir, model_compression_params, "AWQ" in precision, remote_code)
